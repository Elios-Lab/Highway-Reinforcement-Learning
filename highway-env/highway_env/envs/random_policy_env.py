...
class RandomPolicy(AbstractEnv):
    ...
    #method that is called the colab in the last cell and that returns a random action among those available
    def action_random(self):
        actions = AbstractEnv.get_available_actions(self)
        random_action = IDMVehicle.get_random_actions(self, actions = actions)
...
register(
    id='random-policy-v0',
    entry_point='highway_env.envs:RandomPolicy',
)
