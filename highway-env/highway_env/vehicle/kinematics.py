...
class Vehicle(object):
    ...
    WIDTH = 2.0
    """ Vehicle width [m] """
    DEFAULT_SPEEDS_2 = [18, 21]
    DEFAULT_SPEEDS_1 = [23, 25]
    ...
    @classmethod
    def make_on_lane(cls, vehicle_id: int, road: Road, lane_index: LaneIndex, longitudinal: float, speed: float = 0) -> "Vehicle":
        """
        ...
        return cls(vehicle_id, road, lane.position(longitudinal, 0), lane.heading_at(longitudinal), speed)

    @classmethod
    def create_random(cls, vehicle_id: int, road: Road, speed: float = None, lane_id: Optional[int] = None, spacing: float = 1) \
            -> "Vehicle":
        """
        ...
        _id = lane_id if lane_id is not None else random.choice(index_lane)
        lane = road.network.get_lane((_from, _to, _id))

        if speed is None:
            if _id == 2:
                speed = road.np_random.uniform(Vehicle.DEFAULT_SPEEDS_2[0], Vehicle.DEFAULT_SPEEDS_2[1])
                default_spacing = space*speed
            if _id == 1:
                speed = road.np_random.uniform(Vehicle.DEFAULT_SPEEDS_1[0], Vehicle.DEFAULT_SPEEDS_1[1])
                default_spacing = space*speed
            else:
                speed = road.np_random.uniform(Vehicle.DEFAULT_SPEEDS_1[0], Vehicle.DEFAULT_SPEEDS_1[1])
                default_spacing = space*speed
        ...
        v = cls(vehicle_id, road, lane.position(x0, 0), lane.heading_at(x0), speed)
        return v

    @classmethod
    def create_from(cls, vehicle_id: int, vehicle: "Vehicle") -> "Vehicle":
        """
        ...
        """
        v = cls(vehicle_id, vehicle.road, vehicle.position, vehicle.heading, vehicle.speed)
        return v
