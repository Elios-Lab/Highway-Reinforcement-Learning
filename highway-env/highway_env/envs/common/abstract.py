...
class AbstractEnv(gym.Env):
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        if terminal == True: #only if the episode is over
            #to call the statistics function only from simulation envs
            #(by default RL_policy_env has env = 10 because it is used for training)
            a = random.choice(range(3, 10, 3))
            self.config["vehicle_density"] = a/10

            if self.config["env"] == 0 or self.config["env"] == 1 or self.config["env"] == 2: 
                Statistics.statistics(self, distance_algo = self.config["SafeDistance"]) #it is only invoked when an episode ends
    ...
    def get_available_actions(self) -> List[int]:
        ...
        for l_index in self.road.network.side_lanes(self.vehicle.lane_index):
            if l_index[2] < self.vehicle.lane_index[2] \
                    ...
                    and self.vehicle.lane_index[2] != 0:
                actions.append(self.action_type.actions_indexes['LANE_LEFT'])
            if l_index[2] > self.vehicle.lane_index[2] \
                    and self.road.network.get_lane(l_index).is_reachable_from(self.vehicle.position) \
                    and self.action_type.lateral \
                    and self.vehicle.lane_index[2] != self.config["lanes_count"]-1:
